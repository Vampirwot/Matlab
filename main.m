...
[Tmax] = getTmax(h, m_exp, ms_exp, ratioF, Nt);

disp('2 этап. Вычисление экспериментального временного распределения импульса излучения');
disp('---------------------------------------------------------------------------------');
[t, F] = calc(h, m_exp, ms_exp, Tmax, Nt);
disp(['Полная энергия временного распределения = ' num2str(sum(F)*(t(2)-t(1))) ' Дж']);
...
[ma_teor, ms_teor] = calcMaMs(F, ma_exp, ms_exp, h, Tmax, Nt, maxMa, maxMs, dMa, dMs);

m_teor = ma_teor + ms_teor;

[t, F_exp] = calc(h, m_teor, ms_teor, Tmax, Nt); 
