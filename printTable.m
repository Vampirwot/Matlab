function printTable(columns,varargin)
   ...
   for i = 1:length(columns)
      ...
      columnTitle = strcat(columnTitle, columns{i},char({repmat((' '),1,1+maxLength(i)-getLen(columns{i}))}),' |');
   end
   disp(border);
   disp(columnTitle);
   disp(border);
   ...
   for i = 1:length(varargin)
       for j = 1:length(varargin{i})
         line = strcat(line, varargin{i}(j),char({repmat((' '),1,1+maxLength(j)-getLen(varargin{i}(j)))}),' |');
       end
       disp(line);
       line = '| ';
   end
